...
const deleteComment = () => {
    form.delete(route('posts.comments.delete', selectedCommentId.value), {
        onSuccess: () => {
...
<template>
    <button type="button" @click="confirmDeleteComment(props.commentId)"
        class="flex items-center text-sm text-gray-500 hover:bg-gray-200 px-2 py-1 hover:rounded-md dark:text-gray-400 font-medium">
    ...
    <div v-if="showModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
        <div class="bg-white dark:bg-gray-900 p-6 rounded-lg shadow-lg max-w-sm">
            ...
            <div class="mt-4 flex justify-end">
                ...
                <button @click="deleteComment" class="px-4 py-2 bg-red-600 text-white rounded">Confirm</button>
            </div>
        </div>
    </div>
</template>
